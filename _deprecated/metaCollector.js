...
const log = debug('@pq:meta')

// const PRODUCTION_META_PAGE_CONTENT = `
// # &nbsp;
...
const createMeta = () => {
  ...
  hookDocFiles.forEach((file, index) => {
    if (file === 'meta.mdx' || file.includes('index.md') || file.includes('.js')) return
    // if(index > 1) return;
    ...
    requiredHeadings.forEach((rh, i) => {
      ...
      if (!new RegExp(rh).test(fileContents)) {
        ...
        const match1 = new RegExp(requiredHeadings[i]).exec(fileContents).index
        let match2

        const m = new RegExp(requiredHeadings[i + 1]).exec(fileContents)
...
const checkLastMetaTime = () => {
  ...
  const trimmed = metaContent.split('==>')[0].split('<==')[1]
